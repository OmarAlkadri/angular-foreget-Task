...
@Component({
  ...
  templateUrl: './currencyAdding.component.html',
  styleUrl: './currencyAdding.component.css'
})
export class CurrencyAdding {
  [x: string]: any;
  ...
  onSave() {
    ...
    if (this.nameControl.valid) {
      ...
      if (!this.data) {
        this.apiService.addServices(shipmentItems, "Currencies").subscribe(data => {
          console.log(data)
        })
      }
      else {
        shipmentItems.id = this.data.id,
          this.apiService.updateServices(shipmentItems, this.data.id, 'Currencies').subscribe({
            next: data => {
      ...
      setTimeout(() => { this.router.navigate(['home/currency-list']) }, 1000)
    } else {
