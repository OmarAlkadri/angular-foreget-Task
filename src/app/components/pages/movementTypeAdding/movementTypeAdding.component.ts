...
@Component({
  ...
  templateUrl: './movementTypeAdding.component.html',
  styleUrl: './movementTypeAdding.component.css'
})
export class MovementTypeAdding {
  [x: string]: any;
  ...
  onSave() {
    ...
    if (this.nameControl.valid) {
      ...
      if (!this.data) {
        this.apiService.addMovementType(shipmentItems).subscribe(data => {
          console.log(data)
        })
      }
      else {
        shipmentItems.id = this.data.id,
          this.apiService.updateMovementType(shipmentItems, this.data.id).subscribe({
            next: data => {
      ...
      setTimeout(() => { this.router.navigate(['home/movementtype-list']) }, 1000)
    } else {
