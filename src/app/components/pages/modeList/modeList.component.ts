...
@Component({
  ...
  templateUrl: './modeList.component.html',
  styleUrl: './modeList.component.css',
  imports: [
...
export class modeList implements OnInit, OnDestroy {
  data: any[] = [];
  ...
  ngOnInit() {
    this.modesSub = this.apiService.getModes().subscribe(data => {
      this.data = data
    });
    this.editSettings = {
      allowEditing: false,
  ...
  clickHandler = (args: any) => {
    if (args.item.properties.text === 'Add') {
      this.router.navigate(['home/mode-adding']);
    }
  }

  EditModeItem(data: any) {
    const tempData = data
  ...
  deleteModeItem(Id: any) {
    console.log(Id)
    this.loader.showLoader();
    this.apiService.dalateMode(Id).subscribe({
      next: data => {
