...
@Component({
  ...
  templateUrl: './incotermsList.component.html',
  styleUrl: './incotermsList.component.css',
  imports: [
...
export class IncotermsList implements OnInit, OnDestroy {
  data: any[] = [];
  ...
  ngOnInit() {
    this.modesSub = this.apiService.getServicess('Incoterms').subscribe(data => {
      this.data = data
    });
    this.editSettings = {
      allowEditing: false,
  ...
  clickHandler = (args: any) => {
    if (args.item.properties.text === 'Add') {
      this.router.navigate(['home/incoterms-adding']);
    }
  }
  ...
  deleteItem(Id: any) {
    ...
    this.apiService.dalateServices(Id, 'Incoterms').subscribe({
      next: data => {
