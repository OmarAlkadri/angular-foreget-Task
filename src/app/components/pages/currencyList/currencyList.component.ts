...
@Component({
  ...
  styleUrl: './currencyList.component.css',
  imports: [
...
export class CurrencyList implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.modesSub = this.apiService.getServicess('Currencies').subscribe(data => {
      this.data = data
    });
    this.editSettings = {
      allowEditing: false,
  ...
  clickHandler = (args: any) => {
    if (args.item.properties.text === 'Add') {
      this.router.navigate(['home/currency-adding']);
    }
  }
  ...
  deleteItem(Id: any) {
    ...
    this.apiService.dalateServices(Id, 'Currencies').subscribe({
      next: data => {
