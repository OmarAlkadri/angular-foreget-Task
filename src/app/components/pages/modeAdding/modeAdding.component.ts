...
@Component({
  ...
  templateUrl: './modeAdding.component.html',
  styleUrl: './modeAdding.component.css'
})
export class modeAdding {
  [x: string]: any;
  ...
  onSave() {
    ...
    if (this.nameControl.valid) {
      ...
      if (!this.data) {
        this.apiService.addMode(shipmentItems).subscribe(data => {
          console.log(data)
        })
      }
      else {
        shipmentItems.id = this.data.id,
          this.apiService.updateMode(shipmentItems, this.data.id).subscribe({
            next: data => {
      ...
      setTimeout(() => { this.router.navigate(['home/mode-list']) }, 1000)
    } else {
