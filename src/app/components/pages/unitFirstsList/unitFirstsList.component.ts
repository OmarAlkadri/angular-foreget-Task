...
@Component({
  ...
  templateUrl: './unitFirstsList.component.html',
  styleUrl: './unitFirstsList.component.css',
  imports: [
...
export class UnitFirstsList implements OnInit, OnDestroy {
  data: any[] = [];
  ...
  ngOnInit() {
    this.modesSub = this.apiService.getServicess('UnitFirsts').subscribe(data => {
      this.data = data
    });
    this.editSettings = {
      allowEditing: false,
  ...
  clickHandler = (args: any) => {
    if (args.item.properties.text === 'Add') {
      this.router.navigate(['home/unitfirst-adding']);
    }
  }
  ...
  deleteItem(Id: any) {
    ...
    this.apiService.dalateServices(Id, 'UnitFirsts').subscribe({
      next: data => {
