...
@Component({
  ...
  templateUrl: './secondUnitsAdding.component.html',
  styleUrl: './secondUnitsAdding.component.css'
})
export class SecondUnitsAdding {
  [x: string]: any;
  ...
  onSave() {
    ...
    if (this.nameControl.valid) {
      ...
      if (!this.data) {
        this.apiService.addServices(shipmentItems, "SecondUnits").subscribe(data => {
          console.log(data)
        })
      }
      else {
        shipmentItems.id = this.data.id,
          this.apiService.updateServices(shipmentItems, this.data.id, 'SecondUnits').subscribe({
            next: data => {
      ...
      setTimeout(() => {
        this.router.navigate(['home/secondunit-list'])
