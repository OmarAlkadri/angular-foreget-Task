...
@Component({
  ...
  templateUrl: './incotermsAdding.component.html',
  styleUrl: './incotermsAdding.component.css'
})
export class IncotermsAdding {
  [x: string]: any;
  ...
  onSave() {
    ...
    if (this.nameControl.valid) {
      ...
      if (!this.data) {
        this.apiService.addServices(shipmentItems, "Incoterms").subscribe(data => {
          console.log(data)
        })
      }
      else {
        shipmentItems.id = this.data.id,
          this.apiService.updateServices(shipmentItems, this.data.id, 'Incoterms').subscribe({
            next: data => {
      ...
      setTimeout(() => { this.router.navigate(['home/incoterms-list']) }, 1000)
    } else {
