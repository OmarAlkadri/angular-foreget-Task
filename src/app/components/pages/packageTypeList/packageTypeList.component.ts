...
@Component({
  ...
  templateUrl: './packageTypeList.component.html',
  styleUrl: './packageTypeList.component.css',
  imports: [
...
export class PackageTypeList implements OnInit, OnDestroy {
  data: any[] = [];
  ...
  ngOnInit() {
    this.modesSub = this.apiService.getServicess('PackageTypes').subscribe(data => {
      this.data = data
    });
    this.editSettings = {
      allowEditing: false,
  ...
  clickHandler = (args: any) => {
    if (args.item.properties.text === 'Add') {
      this.router.navigate(['home/packagetype-adding']);
    }
  }
  ...
  deleteItem(Id: any) {
    ...
    this.apiService.dalateServices(Id, 'PackageTypes').subscribe({
      next: data => {
