...
export class shipmentList implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.shipmentsSub = this.apiService.getShipments().subscribe(data => {
  ...
  clickHandler = (args: any) => {
    if (args.item.properties.text === 'Add') {
      this.router.navigate(['home/shipment-adding']);
    }
  }

  EditShipmentItem(data: any) {
    const tempData = data
  ...
  deleteShipmentItem(Id: any) {
    this.loader.showLoader();
    this.apiService.dalateShipments(Id).subscribe({
      next: data => {
