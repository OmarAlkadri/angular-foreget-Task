...
@Component({
  ...
  templateUrl: './packageTypeAdding.component.html',
  styleUrl: './packageTypeAdding.component.css'
})
export class PackageTypeAdding {
  [x: string]: any;
  ...
  onSave() {
    ...
    if (this.nameControl.valid) {
      ...
      if (!this.data) {
        this.apiService.addServices(shipmentItems, "PackageTypes").subscribe(data => {
          console.log(data)
        })
      }
      else {
        shipmentItems.id = this.data.id,
          this.apiService.updateServices(shipmentItems, this.data.id, 'PackageTypes').subscribe({
            next: data => {
      ...
      setTimeout(() => { this.router.navigate(['home/packagetype-list']) }, 1000)
    } else {
