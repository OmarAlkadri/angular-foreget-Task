...
@Component({
  ...
  templateUrl: './movementTypeList.component.html',
  styleUrl: './movementTypeList.component.css',
  imports: [
...
export class MovementTypeList implements OnInit, OnDestroy {
  data: any[] = [];
  ...
  ngOnInit() {
    this.modesSub = this.apiService.getMovementTypes().subscribe(data => {
      this.data = data
    });
    this.editSettings = {
      allowEditing: false,
  ...
  clickHandler = (args: any) => {
    if (args.item.properties.text === 'Add') {
      this.router.navigate(['home/movementtype-adding']);
    }
  }
  ...
  deleteItem(Id: any) {
    ...
    this.apiService.dalateMovementType(Id).subscribe({
      next: data => {
